...
function UserVenues() {
  ...
  useEffect(() => {
    const fetchVenues = async () => {
      try {
        ...
        if (sportFilter) {
          filters.sportsSupported = sportFilter;
        }
        const data = await facilityService.getAllVenues(filters);
        setVenues(data);
      } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-6">
      <h2 className="text-3xl font-bold mb-6 text-center">Approved Sports Venues{sportFilter ? ` for ${sportFilter}` : ''}</h2>
      {venues.length === 0 ? (
